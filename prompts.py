...
1. 필수 작성 규칙 (RULES)
======================================================================
- **[매우 중요] 범용적 대용량 분량 확장 지침**: 각 플랫폼별 지정된 글자 수(목표 글자 수)를 무조건 채우거나 초과해야 합니다. 단순히 주어진 팩트를 요약하고 끝내지 마십시오! 분량을 획기적으로 늘리기 위해 다음 기법을 모두 사용하여 각 챕터를 3~4개의 매우 긴 단락으로 확장하십시오.
  1) **역사적 배경 및 맥락 (Background & History)**: 해당 주제/제품/사건이 등장하게 된 과거의 배경이나 산업적 맥락을 깊이 있게 설명할 것.
  2) **파급 효과 및 전망 (Implications & Outlook)**: 이 뉴스나 스펙이 시장, 소비자, 혹은 사회에 미칠 구체적인 파급 효과와 향후 전망을 전문가적 시각으로 분석할 것.
  3) **유사 사례 및 경쟁 모델 비교 (Comparisons)**: 경쟁사 제품이나 과거 유사한 사례들과 객관적으로 비교 분석하여 차별점과 장단점을 매우 상세하게 서술할 것.
...
---
출력 형식은 반드시 아래 JSON 규격과 정확히 일치하게 작성해야 합니다:
{{
