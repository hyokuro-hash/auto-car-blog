...
(※주의: JSON 본문 내부에서 큰따옴표를 사용할 경우 반드시 \\" 로 이스케이프 처리하세요.)
{{
...
# 4. 유튜브 영상 자막/설명 기반 핵심 키워드 및 요약 추출 프롬프트
YOUTUBE_ANALYSIS_PROMPT = """
아래 유튜브 영상의 제목, 설명, 그리고 자막(Transcript) 데이터를 기반으로 분석을 진행해 주세요.
이 영상이 다루고 있는 핵심 자동차 또는 주제를 파악하여, 추가적인 구글 뉴스 검색에 사용할 최적의 검색 키워드(영문 차종 이름 권장, 예: "Hyundai Ioniq 5 N", "Toyota GR86" 등)와 영상 내용의 요약을 추출해 주세요.
...
형식은 반드시 다음 JSON 포맷으로 정확히 출력해 주세요:
{{
  "keyword": "[구글 뉴스 검색에 사용할 최적의 핵심 자동차/주제 검색어 (가급적 글로벌 검색을 위해 영문 차종명 권장, 예: Toyota GR86, Hyundai Ioniq 5 등)]",
  "summary": "[유튜브 영상 내용에 대한 핵심 기술적/디자인적 변화점 및 시승 평 요약 (공백 포함 300~500자 내외)]"
}}
"""
