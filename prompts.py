...
{{
  ...
  "naver_content": "네이버 블로그 독자를 위한 스토리텔링 중심의 가독성 높은 마크다운 원고 (위에서 안내한 이미지 태그들을 본문 적재적소에 모두 포함할 것)",
  "tistory_title": "티스토리 블로그용 전문적이고 정보/기술 전달 중심의 객관적 제목",
  "tistory_content": "티스토리 블로그 독자를 위한 기술적 깊이와 객관적 팩트 분석 중심의 마크다운 원고 (위에서 안내한 이미지 태그들을 본문 적재적소에 모두 포함할 것)",
  "wordpress_title": "워드프레스용 구글 검색 노출(SEO)에 최적화된 키워드 중심 제목 (반드시 한국어로 작성)",
  "wordpress_content": "[매우 중요: 절대 영어를 쓰지 말고 무조건 100% 한국어로만 작성하세요] 워드프레스 독자 및 구글 검색 노출(SEO)을 위한 체계적인 계층 구조 중심 마크다운 원고 (위에서 안내한 이미지 태그들을 본문 적재적소에 모두 포함할 것)"
}}
